...
export default function ChatScreen({ route, navigation }) {
  ...
  const handleContentSizeChange = () => {
    ...
    const scrollToBottom = () => {
      ...
      const contentHeight = flatListRef.current?.contentSize?.height || 0;
      const layoutHeight = flatListRef.current?.layoutHeight || 0;

      // Eğer typing varsa ekstra padding ekleyelim
      const extraOffset = friendTyping ? 60 : 0;

      flatListRef.current.scrollToOffset({
        offset: contentHeight + extraOffset,
        animated: true,
      });
    };
