...
export default function ChatScreen({ route }) {
  ...
  const handleEditMessage = async (item, newText) => {
    ...
    await updateDoc(doc(messagesRef, item.id), {
      text: newText,
      edited: true, // sadece bu alan eklensin
      // status alanına dokunma, mevcut değer korunur
    });
  };
    ...
    return (
      ...
      >
        ...
        {isMe && (
          <Text style={{ fontSize: 12, color: "#ccc", marginTop: 2 }}>
            {item.edited && (
              <MaterialCommunityIcons name="edit" size={24} color="#70ffb0ff" />
            )}
            {formatTime(item.timestamp)}
            {item.status === "sent" && (
              <Ionicons name="checkmark" size={24} color="#2e2e2eff" />
            )}
            {item.status === "delivered" && (
              <Ionicons name="checkmark-done" size={24} color="#2e2e2eff" />
            )}
            {item.status === "seen" && (
              <Ionicons name="checkmark-done" size={24} color="#70ffb0ff" />
            )}
          </Text>
        )}
      </TouchableOpacity>
