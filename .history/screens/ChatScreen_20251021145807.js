  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(chatRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        setFriendInchat(data.information.inChat[friendUid] || false);
        setFriendLastSeen(data.information.lastSeen[friendUid] || null);
        setFriendTyping(data.information.typing[friendUid] || false);
      }
    });
    const unsubscribe1 = onSnapshot(friendRef, (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data();
        setFriendIsOnline(data.isOnline || false);
        setFriendInchat(data.information.lastSeen[friendUid] || null);
        setFriendTyping(data.information.typing[friendUid] || false);
      }
    });
    return () => unsubscribe();
    ...
    >
      ...
      >
        ...
        <Text
          ...
          style={{
            color: friendInchat ? "#70baffff" : theme.text.secondary,
          }}
        >
          {friendInchat
            ? "Sohbette"
