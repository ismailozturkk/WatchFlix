...
export default function ChatScreen({ route, navigation }) {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(chatRef, async (docSnap) => {
      ...
      // Friend durumunu tek map üzerinden al
      setFriendStatus({
        isOnline: Boolean(info.friendIsOnline),
        inChat: Boolean(info.inChat?.[friendUid]),
        typing: Boolean(info.typing?.[friendUid]),
        lastSeen: info.lastSeen?.[friendUid] || null,
      });

      // Mesajları alt koleksiyon yerine direkt snapshot’dan al
      const msgsQuery = query(messagesRef, orderBy("timestamp", "asc"));
  ...
  const memoizedMessages = useMemo(() => {
    return messages.map((msg) => ({
      ...msg,
      parts: msg.text.split(urlRegex).filter(Boolean),
      isLink: (part) => urlRegex.test(part),
    }));
  }, [messages]);
  ...
  const handleContentSizeChange = () => {
    ...
    const interval = setInterval(() => {
      if (flatListRef.current) {
        // friendTyping varsa biraz ekstra kaydır
        flatListRef.current.scrollToEnd({ animated: true });
      }
      tries++;
  ...
  const handleSendSearchResult = async (item) => {
    ...
    const messageData = {
      text: item.title || item.name || "Bilinmiyor", // Mesaj metni
      senderId: currentUser.uid,
      timestamp: serverTimestamp(),
      status: "sent",
      media: {
              ...
              : friendStatus.isOnline
                ...
                : `Son görülme: ${
                    friendStatus.lastSeen
                      ? formatDate(friendStatus.lastSeen)
                      : "bilinmiyor"
                  }`}
          </Text>
        </View>
        ...
        >
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  ...
                  style={{ padding: 5, borderColor: theme.border }}
                >
          ...
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            <View style={{ marginHorizontal: 10 }}>
              {searchOption && (
                <Feather name="search" size={24} color={theme.colors.green} />
              )}
              {textLink && (
                <AntDesign
                  name="link"
                  size={24}
                  color={theme.notesColor.aqua}
                />
              )}
            </View>
            <TextInput
                ...
                {selectedMessage.media && (
                  ...
                  >
                    ...
                    <Text
                      ...
                      numberOfLines={6}
                    >
                      {selectedMessage.media?.overview}
                    </Text>
                  </View>
                )}
