...
export default function MovieDetails({ navigation, route }) {
  ...
  const HALF_HEIGHT = SCREEN_HEIGHT * 0.6;
  const FULL_HEIGHT = SCREEN_HEIGHT * 0.9;

  useEffect(() => {
    if (visible) {
      translateY.value = withSpring(HALF_HEIGHT, { damping: 20 });
    }
  }, [visible]);
  ...
  const onGestureEvent = Animated.useAnimatedGestureHandler({
    ...
    onEnd: () => {
      // Snap to HALF or FULL
      if (translateY.value < HALF_HEIGHT + 50) {
        translateY.value = withSpring(FULL_HEIGHT, { damping: 20 });
      } else if (translateY.value < FULL_HEIGHT - 50) {
        translateY.value = withSpring(HALF_HEIGHT, { damping: 20 });
      } else {
        runOnJS(closeModal)();
      }
    ...
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        <PanGestureHandler
          ...
          onGestureEvent={(e) => {
            "worklet";
            const next = translateY.value + e.nativeEvent.translationY;
            translateY.value = Math.max(HALF_HEIGHT, next);
            if (next > SCREEN_HEIGHT * 0.75) runOnJS(closeModal)();
          }}
        >
          <Animated.View
            style={[
