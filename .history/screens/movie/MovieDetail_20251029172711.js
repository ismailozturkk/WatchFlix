...
export default function MovieDetails({ navigation, route }) {
  ...
  const translateY = useSharedValue(SCREEN_HEIGHT); // Başlangıçta ekran dışında
  const panRef = useRef();

  const MIN_HEIGHT = SCREEN_HEIGHT * 0.3; // mini
  const HALF_HEIGHT = SCREEN_HEIGHT * 0.6; // yarı
  const FULL_HEIGHT = SCREEN_HEIGHT * 0.9; // full
