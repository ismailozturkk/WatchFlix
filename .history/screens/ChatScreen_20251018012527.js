...
export default function ChatScreen({ route }) {
  ...
  const renderItem = ({ item }) => {
    ...
    // Regex ile metni linklere ve normal metinlere ayır
    ...
    return (
      <View style={[styles.message, isMe ? styles.myMsg : styles.friendMsg]}>
        <Text
          style={[styles.messageText, { color: "#fff" }]} // Metin rengi her zaman beyaz
        >
          {parts.map((part, index) =>
            urlRegex.test(part) ? (
              // Eğer kısım bir link ise, tıklanabilir ve farklı renkte yap
              <>
                <Text
                  key={index}
                ...
                <EvilIcons name="external-link" size={22} color="black" />
              </>
            ) : (
              // Normal metin
              part
