...
export default function ChatScreen({ route }) {
  ...
  const sendMessage = async () => {
    ...
    if (editingMessage) {
      ...
      messageRef = doc(messagesRef, editingMessage.id);
    ...
    await setDoc(
      chatInfoRef,
      {
        ...
        totalMessages: (messages.length || 0) + 1,
        participants: [currentUser.uid, friendUid],
        updatedAt: serverTimestamp(),
      },
      { merge: true } // Var olan verileri koru ve sadece güncelle
    );
