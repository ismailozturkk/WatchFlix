    ...
    useEffect(() => {
      ...
      const unsubscribe = onSnapshot(docRef, (docSnap) => {
        if (docSnap.exists()) {
          ...
          const favoriteTv = data.favorites || [];
          const watchListTv = data.watchList || [];

          setWatched(
            watchedTv.some((tv) => tv.id === item.id && tv.type === "tv")
          );
          setFavorite(
            favoriteTv.some((tv) => tv.id === item.id && tv.type === "tv")
          );
          setWatchList(
            watchListTv.some((tv) => tv.id === item.id && tv.type === "tv")
          );
        } else {
          setWatched(false);
          setFavorite(false);
