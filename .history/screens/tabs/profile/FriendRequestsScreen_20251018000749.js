...
export default function FriendRequestsScreen() {
  ...
  const renderItem = ({ item }) => (
    <SwipeCard
      onSwipeRight={() => handleAccept(item)} // sağ kaydır: kabul et
      onSwipeLeft={() =>
        tab === "received" ? handleDecline(item) : handleCancelSent(item)
      } // sol kaydır: reddet veya iptal et
    >
      <View style={[styles.requestItem, { backgroundColor: theme.secondary }]}>
        ...
        {tab === "received" ? (
          <View style={{ flexDirection: "row", gap: 10 }}>
            <TouchableOpacity
              style={[styles.btn, { backgroundColor: "#30a75e" }]}
              onPress={() => handleAccept(item)}
            >
              <Text style={{ color: "#fff", fontWeight: "600" }}>Kabul Et</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.btn, { backgroundColor: "#c44f4f" }]}
          ...
          >
            <Text style={{ color: "#fff", fontWeight: "600" }}>İptal Et</Text>
          </TouchableOpacity>
        )}
      </View>
    </SwipeCard>
