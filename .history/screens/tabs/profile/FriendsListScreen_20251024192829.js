...
export default function FriendsListScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    const docRef = doc(db, "Lists", selectedFriend.uid);
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        // Sadece 'listVisible' true olanları filtrele
        const friendData = docSnap.data();
        const visibleLists = friendData.listVisible?.filter(
          (list) => Object.values(list)[0] === true
        );
        const visibleListNames = visibleLists?.map((list) => Object.keys(list)[0]);
    ...
    // Component unmount olduğunda veya selectedFriend değiştiğinde dinleyiciyi kaldır
    return () => unsubscribe();
  }, [selectedFriend]);
  ...
  const handleDelete = async (friend) => {
    try {
      ...
      const currentUserDataForFriend = friendSnap.data().friends.find(f => f.uid === user.uid);
      if(currentUserDataForFriend) {
        await updateDoc(friendRef, { friends: arrayRemove(currentUserDataForFriend) });
      }
  ...
  const renderFriend = ({ item }) => (
    ...
    >
      <TouchableOpacity
        onPress={() => setSelectedFriend(selectedFriend?.uid === item.uid ? null : item)}
      ...
      {selectedFriend?.uid === item.uid && (
        <FlatList
          data={[
            ...protectedLists
              .map((name) => friendLists.find(([listName]) => listName === name))
              .filter(Boolean),
            ...friendLists
          ...
          renderItem={({ item: listItem }) => {
            ...
            return (
              <TouchableOpacity
                activeOpacity={0.8}
                onPress={() => {
                  // Arkadaşın listesine gitmek için özel bir ekran veya parametre gerekebilir
