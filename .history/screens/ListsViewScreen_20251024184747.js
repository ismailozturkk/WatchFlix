...
export default function ListsViewScreen({ navigation }) {
  ...
  const [listVisible, setListsVisible] = useState(false); // Silme modalı için state
  useEffect(() => {
    if (!user?.uid) return;
    const addToListVisible = async (watchedStatus, listName) => {
      const userRef = doc(db, "Users", user.uid);
      await updateDoc(userRef, {
        listVisible: arrayUnion({ listName: watchedStatus }),
      });
    };
    return () => {
      addToListVisible();
  ...
  // Firestore'dan çek
  useEffect(() => {
    const fetchListVisible = async ({ user }) => {
      if (!user) return;
      const userRef = doc(db, "Users", user.uid);
              ...
              >
                ...
                >
                  <TouchableOpacity
                    ...
                    onPress={() => setListsVisible(!listVisible)}
                  >
                    <Ionicons
                      name={listVisible ? "eye" : "eye-outline"}
                      size={20}
                      color={
                        listVisible ? theme.colors.green : theme.colors.red
                      }
                    />
                  </TouchableOpacity>
                  <Text
