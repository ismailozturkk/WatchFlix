...
export default function RegisterScreen({ navigation }) {
  ...
  const validateUsername = (text) => {
    // Sadece KÜÇÜK harf, rakam ve alt çizgi (_) olabilir, boşluk yok
    const regex = /^[a-z0-9_]+$/;
  ...
  const handleUsernameChange = (text) => {
    ...
    setUsername(lowercasedText);
    setText(lowercasedText);

    if (!lowercasedText || lowercasedText.length < 3) {
      setUsernameAvailable(null); // geçersiz username
      return;
    } else if (lowercasedText.length > 20) {
      setUsernameAvailable(false); // çok uzun
      return;
    } else if (!validateUsername(lowercasedText)) {
      setUsernameAvailable(false); // geçersiz karakter
      return;
    }
    ...
    usernameTimeout = setTimeout(() => {
      checkUsername(lowercasedText); // Firestore kontrolü
    }, 500);
  };
      ...
      <View style={[styles.container, { backgroundColor: theme.primary }]}>
        ...
        >
          ...
          {usernameAvailable === null && (
            ...
            >
              (3-20 karakter, boşluk yok, sadece harf, rakam ve _ )
            </Text>
          )}
        </View>
