...
export default function RegisterScreen({ navigation }) {
  ...
  const createAccount = async () => {
    if (!name || !lastname) {
      Toast.show({
        type: "error",
        text1: "Lütfen isim ve soyisim giriniz!",
      });
      return;
    }
    ...
    if (password !== passwordAgain) {
      Toast.show({
        type: "error",
        text1: "Şifreler eşleşmiyor!",
      });
      return;
    }

    const username = (name + lastname).toLowerCase(); // Kullanıcı adı oluştur
    setIsloading(true);
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        Toast.show({
          type: "error",
          text1: "Bu kullanıcı adı zaten alınmış!",
        });
        setIsloading(false);
        return;
      }
      ...
      await setDoc(userRef, {
        ...
        avatar: null,
        createdAt: new Date(),
        friends: [], // arkadaşlık sistemi için başlangıç
        friendRequestsSent: [],
