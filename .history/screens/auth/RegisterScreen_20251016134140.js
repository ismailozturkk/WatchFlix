...
export default function RegisterScreen({ navigation }) {
  ...
  const checkUsername = async (name) => {
    console.log("Kullanıcı adı kontrol ediliyor:", name.length);
  ...
  const handleUsernameChange = (text) => {
    setUsername(text);
    setUsernameAvailable(null); // kontrol edilmedi

    if (usernameTimeout) clearTimeout(usernameTimeout);
    usernameTimeout = setTimeout(() => {
      checkUsername(text.toLowerCase());
    }, 500); // 500ms sonra sorgu
  };
  ...
  const createAccount = async () => {
    if (!username.length < 3) {
      Toast.show({ type: "error", text1: "Lütfen kullanıcı adı giriniz!" });
      return;
    }
