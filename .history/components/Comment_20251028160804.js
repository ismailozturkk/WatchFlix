...
const CommentItem = memo(
  ({
    ...
    const handleTranslate = async (textToTranslate) => {
      ...
      setLoading(true);
      const result = await translateText(textToTranslate, "en");
      setTranslated(result);
      setShowOriginal(false); // Çeviri sonrası çevrilmiş metni göster
      setLoading(false);
    };
    return (
      <View
            ...
            {item.isSpoiler && !showSpoiler ? (
              ...
              >
                <Text style={{ marginTop: 4, color: theme.text.primary }}>
                  {showOriginal ? item.text : translated}
                </Text>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                  {loading ? (
                    ...
                    <TouchableOpacity onPress={() => handleTranslate(item.text)}>
                      <Text style={{ color: theme.text.secondary, fontSize: 12 }}>çevir</Text>
                    </TouchableOpacity>
                  ) : (
                    <TouchableOpacity onPress={() => setShowOriginal(true)}>
                      <Text style={{ color: theme.text.secondary, fontSize: 12 }}>orijinali göster</Text>
                    </TouchableOpacity>
                  )}
