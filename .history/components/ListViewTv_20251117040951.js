...
const ListViewTv = ({
  ...
  const onPressIn = (listName) => {
    ...
    // Mevcut animasyonu durdur ve hemen yeni değeri ayarla
    scaleValues[listName].stopAnimation();
    Animated.timing(scaleValues[listName], {
      toValue: 0.85, // Daha belirgin bir küçülme efekti için
      duration: 100, // Hızlı tepki süresi
  ...
  const onPressOut = (listName) => {
    ...
    Animated.spring(scaleValues[listName], {
      toValue: 1,
      friction: 3, // Daha yumuşak bir yaylanma
      tension: 150, // Daha yumuşak bir yaylanma
      useNativeDriver: true,
    }).start();
  };
    ...
    >
      <TouchableOpacity
        ...
        onPress={() => {
          ...
          }, 50); // Animasyonun başlaması için küçük bir gecikme
        }}
      >
        <Animated.View
          style={[
