...
export const ProfileScreenProvider = ({ children }) => {
  ...
  const getRankColor = (totalWatchedTime, type = "tv") => {
    const ranks = [
      {
        limit: 20160,
    ...
    // Fark sadece renklerin sırayla atanması
    const [c1, c2, c3] = rank
      ? rank.colors
  ...
  const { borderColorTv, borderColor2Tv, shadowColorTv } = getRankColor(
    totalMinutesTimeTv,
    "tv"
  );

  const { borderColorMovie, borderColor2Movie, shadowColorMovie } =
    getRankColor(totalMinutesTime, "movie");
