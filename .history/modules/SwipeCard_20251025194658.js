...
const SwipeCard = ({ children, leftButton, rightButton }) => {
  ...
  const onHandlerStateChange = (event) => {
    if (event.nativeEvent.oldState === State.ACTIVE) {
      ...
      // Buton otomatik tetikleme
      if (translationX > SWIPE_LIMIT && allowLeft) {
        leftButton.onPress();
      } else if (translationX < -SWIPE_LIMIT && allowRight) {
        rightButton.onPress();
      }

      // Kartı animasyonla geri çek
      Animated.spring(translateX, {
        toValue: 0,
        useNativeDriver: true,
      }).start();
    ...
    <View style={styles.container}>
      ...
      {allowLeft && (
        <Animated.View
          ...
          pointerEvents={canPressLeft ? "auto" : "none"} // <--- burada
        >
          <TouchableOpacity
            style={styles.button}
            activeOpacity={0.8}
      ...
      {allowRight && (
        <Animated.View
          ...
          pointerEvents={canPressRight ? "auto" : "none"} // <--- burada
        >
          <TouchableOpacity
            style={styles.button}
            activeOpacity={0.8}
